...
class Site_rule {
  ...
  static findById(id, result) {
    dbConn.query(
      "SELECT * FROM site_rule WHERE id = ?",
      id,
      function (err, res) {
  ...
  static findByContactId(id, result) {
    dbConn.query(
      `SELECT s.name, s.id, s.url, s.domain, s.icon_path, s.private_key, s.public_key, s.active, s.url_amazon
                        FROM site AS s
  ...
  static update(id, site_rule, result) {
    dbConn.query(
      `UPDATE site_rule 
                        ...
                        WHERE id = ?`,
      [
        site_rule.first_name,
        site_rule.last_name,
        site_rule.email,
        site_rule.function,
        site_rule.icon_path,
        site_rule.id_account,
        site_rule.created_by,
        site_rule.modified_by,
        site_rule.last_modification,
        id,
      ],
      function (err, res) {
        err ? result(err, null) : result(null, res);
